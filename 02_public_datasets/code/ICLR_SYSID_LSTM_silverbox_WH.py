...
# benchmark example: Silverbox/ Wiener-Hammerstein
# ------------------------------------------------------------
...
# --- DATASETS --- #
for benchmark_mode in range(0,2):

    if benchmark_mode == 0:
        path_data_train = "data/data_silverbox_train_20000.csv"
        path_data_val = "data/data_silverbox_val_10000.csv"
        path_data_test = "data/data_silverbox_test_10000.csv"
        data_set_name = "silverbox"

    else:
        path_data_train = "data/data_WienerHammerBenchmark_train_20000.csv"
        path_data_val = "data/data_WienerHammerBenchmark_val_10000.csv"
        path_data_test = "data/data_WienerHammerBenchmark_test_10000.csv"
        data_set_name = "wienerhammerstein"
    ...
    for train_mode in range(1,2):
        ...
        if train_mode == 0:
            ...
            m_number_of_vals_list = [20,30,40,40]
            batch_size_b = 100
            num_of_epochs = 400 #200
            ...
            training_type = "BPTT"
            stateful = False
            timesteps_per_sequence_N_list = [20000]
        ...
        # --- load training data --- #
        u_train, y_train, u_scaler, y_scaler, length_train = load_data_training(path_data_train, scaler)
        X_train_eval, _, _ = create_sequences_io_many_to_many_shift_by_one_mapping(u_train, y_train, length_train)

        # --- load validation data --- #
        u_val, y_val, length_val = load_data_testing(path_data_val, u_scaler, y_scaler)
        X_val, Y_val, _ = create_sequences_io_many_to_many_shift_by_one_mapping(u_val, y_val, length_val)

        # --- load test data --- #
        u_test, y_test, length_test = load_data_testing(path_data_test, u_scaler, y_scaler)
        ...
        for timesteps_per_sequence_N in timesteps_per_sequence_N_list:
            ...
            if __name__ == "__main__":
                ...
                for m_loss_parameter in m_parameters_list:
                    ...
                    model_file_name = f'model_h{hidden_size}_N{timesteps_per_sequence_N}_m{m_loss_parameter}_{training_type}'
